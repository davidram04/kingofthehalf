function isSignaturePadMessage(data) {
  try {
    if (!data) {
      return false;
    }
    var eventData = typeof data === 'string' ? JSON.parse(data) : data;
    return !!(eventData && typeof eventData.messageType === 'string' && eventData.messageType === 'buttonClick');
  } catch (error) {
    return false;
  }
}
...
jQuery(document).ready(function() {
  ...
  var signaturePadMessageBus = null;

  function getSignaturePadMessageBus() {
    if (signaturePadMessageBus) {
      return signaturePadMessageBus;
    }
    ...
    if (typeof messageBusFactory !== 'function') {
      console.warn('Message bus factory is not a function, skipping signature pad message bus');
      return null;
    }

    signaturePadMessageBus = messageBusFactory({
      allowedOrigins: [window.location.origin],
      allowedSources: function() {
        return [window.parent, window].filter(Boolean);
      }
    });
  ...
  jQuery('.pad').each(function(idx, el) {
    ...
    var messageBus = getSignaturePadMessageBus();
    if (messageBus) {
      messageBus.onRaw(function(event) {
        if (!isSignaturePadMessage(event.data)) {
          return;
        }
